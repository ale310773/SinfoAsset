...
Public Class frmMasterKondisi
    ...
    Public Sub New()
        ...
        GroupPanel1.Text = "Master Kondisi"

        disabledpropertyForm()
        ButtonCRUDWhenDefault()

        'If DataGridViewKondisi.RowCount > 0 Then
        '    LoadsettingKondisi()
        '    Try

        '        DataGridViewKondisi.Rows(0).Selected = True
        '    Catch ex As Exception
        ...
        Try
            ...
            '    LoadsettingItem()
            'End If
            If (DataGridViewKondisi.RowCount > 0 And DataGridViewKondisi.ColumnCount > 0) Then
    ...
    Private Sub disabledpropertyForm()
        txtkodekondisi.ReadOnly = True
        txtkodekondisi.ForeColor = Color.Black
        txtkodekondisi.BackColor = Color.Gold

        txtnamaKondisi.ReadOnly = True
        txtnamaKondisi.ForeColor = Color.Black
        txtnamaKondisi.BackColor = Color.Gold
    ...
    Private Sub enabledpropertyForm()
        If chrStatusSave = "ADD" Then
            txtkodeKondisi.ReadOnly = False
        Else
            txtkodeKondisi.ReadOnly = True
        End If

        txtkodeKondisi.ForeColor = Color.Black
        txtkodeKondisi.BackColor = Color.GreenYellow

        txtnamaKondisi.ReadOnly = False
        txtnamaKondisi.ForeColor = Color.Black
        txtnamaKondisi.BackColor = Color.GreenYellow
    ...
    Private Sub ButtonCRUDWhenDefault()
        ...
        'DataGridViewKondisi.Enabled = True
        'txtSearch.Enabled = True
    End Sub
    ...
    Private Sub whencancel2()
        ...
        'End If
    End Sub
    Private Sub LoadsettingKondisi()
        ' refreshKondisi()
        refreshKondisi2()
        SetFormattingGridViewKondisi()
        txtSearch.Text = ""
    End Sub
    'Private Sub refreshKondisi()
    '    Dim cstrSearch As String = "%" + RTrim(LTrim(txtSearch.Text)) + "%"
    '    Dim cstrsql As String

    '    cstrsql = " select id,kode_Kondisi,nama_Kondisi from tbl_master_Kondisi " & _
    '  " where ( tbl_master_Kondisi.kode_Kondisi like '" & cstrSearch & "' OR " & _
    '        " dbo.tbl_master_Kondisi.nama_Kondisi " & " like '" & cstrSearch & "' )" & _
    ' "  ORDER BY tbl_master_Kondisi.kode_Kondisi "

    '    Dim adapter As New SqlDataAdapter(cstrsql, ModuleApp.Koneksi)
    '    Me.DataGridView1.Columns.Clear()
    '    Dim dt As New DataTable("Kondisi")
    '    adapter.Fill(dt)
    '    DataGridViewKondisi.DataSource = dt
    '    Dim tblsalestenant As DataTable
    '    tblsalestenant = dt
    'End Sub
    Private Sub refreshKondisi2()

        Dim cstrParamSearch As New SqlClient.SqlParameter
        Dim cstrsearch As String = RTrim(LTrim(txtSearch.Text))
        Dim ocmdLasTransaction As SqlCommand = New SqlCommand("usp_search_master_Kondisi_sel", ModuleApp.Koneksi)
        ...
        DataGridViewKondisi.DataSource = oTblLasTransaction


    End Sub
    Private Sub SetFormattingGridViewKondisi()


        Dim ckode_Kondisi As System.Windows.Forms.DataGridViewTextBoxColumn = New System.Windows.Forms.DataGridViewTextBoxColumn
        Dim cnama_Kondisi As System.Windows.Forms.DataGridViewTextBoxColumn = New System.Windows.Forms.DataGridViewTextBoxColumn
        ...
        ckode_Kondisi.Name = "kode_Kondisi"
        ckode_Kondisi.HeaderText = "Kode Kondisi"
        ckode_Kondisi.DataPropertyName = "kode_Kondisi"
        ckode_Kondisi.Width = 100
        ckode_Kondisi.Visible = True
        ckode_Kondisi.ReadOnly = True
        ckode_Kondisi.DefaultCellStyle.BackColor = Color.BlanchedAlmond

        cnama_Kondisi.Name = "nama_Kondisi"
        cnama_Kondisi.HeaderText = "Nama Kondisi"
        cnama_Kondisi.DataPropertyName = "nama_Kondisi"
        cnama_Kondisi.Width = 100
        cnama_Kondisi.Visible = True
        cnama_Kondisi.ReadOnly = True
        cnama_Kondisi.DefaultCellStyle.BackColor = Color.BlanchedAlmond
        ...
        DataGridViewKondisi.AllowUserToAddRows = False
        DataGridViewKondisi.AllowUserToDeleteRows = True
        DataGridViewKondisi.AutoGenerateColumns = False
        DataGridViewKondisi.Columns.Clear()

        '"select  sales_date,tenant_id, kode , menu , price , ppn , price_ppn , qty_sales , total_sales from tbl_temp_sales ", Module1.Koneksi)
        DataGridViewKondisi.Columns.AddRange(New System.Windows.Forms.DataGridViewColumn() _
        { _
        ckode_Kondisi, _
        cnama_Kondisi, _
        cid, _
        ...
        Me.DataGridViewKondisi.AllowUserToAddRows = False

        'Me.DataGridViewKondisi.Columns.Item(3).DefaultCellStyle.Format = "n2"      'nominal 
        '   setting format datetime dataGridView1.Columns[0].DefaultCellStyle.Format = "dd/MM/yyyy";
        'Me.DataGridViewKondisi.Columns.Item(4).DefaultCellStyle.Format = "dd/MM/yyyy"      'net sales
        'Me.DataGridViewKondisi.Columns.Item(5).DefaultCellStyle.Format = "dd/MM/yyyy"      'net sales
    ...
    Private Sub GoToFirstRecord()
        ...
        DataGridViewKondisi.CurrentCell = DataGridViewKondisi.Rows(0).Cells(0)

        If IsDBNull(DataGridViewKondisi.Rows(0).Cells(0).Value) Then
            Return
        End If
        Try

            idTable = Convert.ToString(DataGridViewKondisi.Rows(0).Cells(2).Value)
            txtkodeKondisi.Text = DataGridViewKondisi.Rows(0).Cells(0).Value
            txtnamaKondisi.Text = DataGridViewKondisi.Rows(0).Cells(1).Value

            '=============================
            txtcreatedate.Text = DataGridViewKondisi.Rows(0).Cells(3).Value
            txtcreateby.Text = DataGridViewKondisi.Rows(0).Cells(4).Value
            txtlastmodified.Text = DataGridViewKondisi.Rows(0).Cells(5).Value
            txtmodifiedby.Text = DataGridViewKondisi.Rows(0).Cells(6).Value
    ...
    Private Sub whensaverecord()
        ...
        Try
            ...
            End If
            If chrStatusSave = "ADD" Then
                MessageBox.Show("TAMBAH DATA KONDISI SUKSES...", "SAVE RECORDS", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Else
                MessageBox.Show("UPDATE DATA KONDISI SUKSES...", "SAVE RECORDS", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
    ...
    End Sub
    Private Sub UpdaterecordKondisi()

        idTable2 = Convert.ToString(Me.DataGridViewKondisi.CurrentRow.Cells(2).Value)
        xCommand.Connection = sqlconn
        Dim xParamkodeKondisi As New SqlClient.SqlParameter
        Dim xParamnamaKondisi As New SqlClient.SqlParameter
        ...
        xCommand.CommandText = "usp_master_Kondisi_upd#id"
        xCommand.CommandType = CommandType.StoredProcedure

        'parameter yang dikirim melalui objek command
        xParamkodeKondisi = xCommand.Parameters.Add("@id", SqlDbType.UniqueIdentifier, 16)
        'direction yg dipakai mode input
        xParamkodeKondisi.Direction = ParameterDirection.Input
        xCommand.Parameters("@id").Value = New Guid(idTable2)
        'idTable2
        'New Guid(idTable2)

        'parameter yang dikirim melalui objek command
        xParamnamaKondisi = xCommand.Parameters.Add("@nama_Kondisi", SqlDbType.NVarChar, 100)
        'direction yg dipakai mode input
        xParamnamaKondisi.Direction = ParameterDirection.Input
        xCommand.Parameters("@nama_Kondisi").Value = LTrim(RTrim(txtnamaKondisi.Text))
        ...
        xCommand.Parameters.Remove(xParamkodeKondisi)
        xCommand.Parameters.Remove(xParamnamaKondisi)
        xCommand.Parameters.Remove(xParamModifiedBy)
    End Sub
    Private Sub saverecordKondisi()
        ...
        xCommand.CommandText = "usp_master_Kondisi_ins"
        xCommand.CommandType = CommandType.StoredProcedure

        'parameter yang dikirim melalui objek command
        xParamkodeKondisi = xCommand.Parameters.Add("@kode_Kondisi", SqlDbType.NVarChar, 50)
        'direction yg dipakai mode input
        xParamkodeKondisi.Direction = ParameterDirection.Input
        xCommand.Parameters("@kode_Kondisi").Value = LTrim(RTrim(txtkodekondisi.Text))
    ...
    Private Sub WhenDeleteRecord()
        idTable2 = Convert.ToString(Me.DataGridViewKondisi.CurrentRow.Cells(3).Value)
        Dim mResult
        ...
        Else
            ...
            Try
                ...
                'cmd.ExecuteNonQuery()
                Dim cmdinserttenantPOS1 As New SqlCommand("usp_master_Kondisi_del#id", ModuleApp.Koneksi)
                ...
                MsgBox("PENGHAPUSAN DATA KONDISI SUKSES...", MsgBoxStyle.Information)
                LoadsettingKondisi()
    ...
    Private Sub Button_Search_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button_Search.Click
        ...
        txtSearch.Focus()
    End Sub
    Private Sub DataGridViewKondisi_CellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridViewKondisi.CellClick

        If (DataGridViewKondisi.RowCount = 0 & DataGridViewKondisi.ColumnCount = 0) Then
            Return
        End If


        Dim i As Integer
        i = DataGridViewKondisi.CurrentRow.Index

        idTable = Convert.ToString(DataGridViewKondisi.Rows(i).Cells(2).Value)
        txtkodekondisi.Text = DataGridViewKondisi.Rows(i).Cells(0).Value
        txtnamaKondisi.Text = DataGridViewKondisi.Rows(i).Cells(1).Value


        txtcreatedate.Text = DataGridViewKondisi.Rows(i).Cells(3).Value
        txtcreateby.Text = DataGridViewKondisi.Rows(i).Cells(4).Value
        txtlastmodified.Text = DataGridViewKondisi.Rows(i).Cells(5).Value
        txtmodifiedby.Text = DataGridViewKondisi.Rows(i).Cells(6).Value
    End Sub

    Private Sub DataGridViewKondisi_CellDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridViewKondisi.CellDoubleClick
        ...
        Try
            ...
            idTable = Convert.ToString(DataGridViewKondisi.Rows(e.RowIndex).Cells(2).Value)
            txtkodekondisi.Text = DataGridViewKondisi.Rows(e.RowIndex).Cells(0).Value
            txtnamaKondisi.Text = DataGridViewKondisi.Rows(e.RowIndex).Cells(1).Value
            'DateTimePickerStartDate.Value = DataGridViewLokasi.Rows(e.RowIndex).Cells(4).Value
            'DateTimePickerEnDate.Value = DataGridViewLokasi.Rows(e.RowIndex).Cells(5).Value
            'txtNominal.Text = FormatNumber(DataGridViewLokasi.Rows(e.RowIndex).Cells(3).Value, 2, TriState.UseDefault)
            ...
            'recorded
            txtcreatedate.Text = DataGridViewKondisi.Rows(e.RowIndex).Cells(3).Value
            txtcreateby.Text = DataGridViewKondisi.Rows(e.RowIndex).Cells(4).Value
            txtlastmodified.Text = DataGridViewKondisi.Rows(e.RowIndex).Cells(5).Value
            txtmodifiedby.Text = DataGridViewKondisi.Rows(e.RowIndex).Cells(6).Value

        Catch ex As Exception

        End Try
    End Sub
    Private Sub DataGridViewKondisi_CellMouseDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DataGridViewKondisi.CellMouseDoubleClick
    ...
    Private Sub txtkodekondisi_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtkodekondisi.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            txtnamaKondisi.Focus()    'Add Row Table
            'txtKodeItems.Enabled = True
        End If
    End Sub
    Private Sub txtnamaKondisi_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtnamaKondisi.KeyPress
        If e.KeyChar = Chr(Keys.Enter) Then
            ToolStrip1.Focus()
            Button_Save.Select()
            'txtKodeItems.Enabled = True
